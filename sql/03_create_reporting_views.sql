...
SELECT
    ...
    ROUND(AVG(resolution_hours) FILTER (WHERE is_closed), 2)
    ...
    COUNT(*) FILTER (WHERE sla_status = 'OPEN_OVERDUE') AS open_overdue,
    ROUND(
        100.0
        * COUNT(*) FILTER (WHERE sla_status = 'MET')
        / NULLIF(
            COUNT(*) FILTER (WHERE sla_status IN ('MET', 'BREACHED')),
            0
        ),
        2
    ) AS sla_compliance_rate
FROM analytics.tickets
GROUP BY COALESCE(agency_name, agency_code, 'Unknown');
